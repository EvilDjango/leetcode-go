...
func Constructor(matrix [][]int) NumMatrix {
	...
	return NumMatrix{matrix, trees}
}

func buildTree(nums []int, tree []int) {
...
func (this *NumMatrix) Update(row int, col int, val int) {
	...
	for i := col + 1; i < len(this.trees[0]); i += lowBit(i) {
		this.trees[row][i] += delta
	}
...
func (this *NumMatrix) SumRegion(row1 int, col1 int, row2 int, col2 int) int {
	sum := 0
	for i := row1; i <= row2; i++ {
		sum += this.sumRegion(i, col2) - this.sumRegion(i, col1-1)
	}
	return sum
}

// 求row行前col个元素和
func (this *NumMatrix) sumRegion(row, col int) int {
	sum := 0
	for i := col + 1; i > 0; i -= lowBit(i) {
		sum += this.trees[row][i]
	}
	return sum
}

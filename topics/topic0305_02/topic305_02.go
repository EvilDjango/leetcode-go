...
func numIslands2(m int, n int, positions [][]int) []int {
	...
	for i, position := range positions {
		...
		for _, direct := range directions {
			...
			index2 := x*n + y + 1
			if roots[index2] != 0 && merge(roots, index, index2) {
				ans[i]--
			}
		}
	}
...
func merge(roots []int, key1, key2 int) bool {
	root1 := getRoot(roots, key1)
	root2 := getRoot(roots, key2)
	if root1 == root2 {
		return false
	}
	roots[root1] = root2
...
func getRoot(unions []int, key int) int {
